                    ...
                    <select class="form-select mt-2" name="challenge_id" id="challenge_id" required>
                        ...
                        <?php if (!empty($challenges) && is_array($challenges)): ?>
                            <?php foreach ($challenges as $challenge): ?>
                                <option value="<?= htmlspecialchars($challenge->challenge_id) ?>"><?= htmlspecialchars($challenge->name) ?></option>
                            <?php endforeach; ?>
                        <?php else: ?>
                    ...
                    <select class="form-select mt-2" name="user_id" id="user_id" required>
                        ...
                        <?php if (!empty($users) && is_array($users)): ?>
                            <?php foreach ($users as $user): ?>
                                <option value="<?= htmlspecialchars($user->user_id) ?>"><?= htmlspecialchars($user->pseudo) ?></option>
                            <?php endforeach; ?>
                        <?php else: ?>
